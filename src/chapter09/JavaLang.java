...
public class JavaLang {
	public static void main(String[] args) {
		...
		MyClass myClass1 = new MyClass(1,"전창현");
		System.out.println(myClass1);
		// MyClass(ID: 1 / NAME: 전창현)
		// : 객체를 출력하는 경우 자동으로 toString() 호출 - 생략가능
		System.out.println(myClass1.toString());
		
		System.out.println(myClass1.displayInfo());

		MyClass myClass2 = new MyClass(2, "문창배");
		...
		System.out.println(message.length()); // 16 - 문자 뿐만 아니라 공백, 기호까지 포
		
		boolean isEquals = message.endsWith("Merry Christmas! ^^");
		System.out.println(isEquals); // false - 문자열 끼리 비교할때 주소값이 아닌 데이터 값이 동일한지 확인
