...
/*
 * === PHR (개인 건강 기록, Personal Health Record) 솔루선 ===
 * 
 * 1. 프로젝트 개요
...
public class App {
	...
	private static final HealthRecordController healthRecordCintroller = new HealthRecordController();

	private static void displayMenu() {
		System.out.println("\n[메뉴 선택]");
		// cf) 이스케이프 문자
		// : 문자 제어코드, 문자열 내에서 ㅡㄱ수한 기능을 수행하는 문자
		// - 백슬래시(\)를 사용하여 표현
		// \n(줄바꿈), \\(\ 기호 그 자체 \ "(" 기호 그자체) 등
		System.out.println("1. 환자 등록");
		System.out.println("2. 환자 전체 조회");
		System.out.println("3. 환자 단건 조회");
	...
	private static PatientRequestDto createPatientRequiest() {
	...
	private static boolean processChoice(int choice) {
		...
		// 환자 관련 기능
		case 1: {
			PatientRequestDto requestDto = createPatientRequiest();
		...
		case 6: {
			...
			healthRecordCintroller.createRecord(requestDto);
			break;
		}
		case 7: {
			List<RecordResponseDto> records = healthRecordCintroller.getAllRecords();
			if (records.isEmpty()) {
		...
		case 8: {
			...
			List<RecordResponseDto> filteredRecords = healthRecordCintroller.filterRecordsByDiagnosis(diagnosisFilter);
		...
		case 9: {
			long id = getIdInput();
			healthRecordCintroller.deleteRecord(id);
			break;
		}
		case 10: {
